...
/**
 * Whether to show the one-click demo logins. Passed in from the server
 * component (page.tsx), which reads the runtime ENABLE_DEMO_LOGIN env var.
 * Driven by a prop — NOT process.env — so the block is never dead-code
 * eliminated from the client bundle and SSR/hydration always agree.
 */
export function LoginForm({ demoLoginEnabled }: { demoLoginEnabled: boolean }) {
  ...
  return (
    <Card>
      ...
      {/* Demo logins — gated on the demoLoginEnabled prop (server-computed from
          the runtime ENABLE_DEMO_LOGIN var). The server provider gate lives in
          auth.ts and also uses ENABLE_DEMO_LOGIN. */}
      {demoLoginEnabled && (
        <>
          <div className="px-6">
            <div className="relative">
